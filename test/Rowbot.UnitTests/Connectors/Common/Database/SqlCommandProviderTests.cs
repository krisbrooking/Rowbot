...
namespace Rowbot.UnitTests.Connectors.Common.Database
{
    public class SqlCommandProviderTests
    {
        ...
        private IEnumerable<RowUpdate<UpdateEntity>> GetValidEntities()
        {
            return new List<RowUpdate<UpdateEntity>>
            {
                new RowUpdate<UpdateEntity>(new UpdateEntity(1, 1, "Alice"), new List<FieldDescriptor> { new FieldDescriptor(typeof(UpdateEntity).GetProperty("Name")!) }),
                new RowUpdate<UpdateEntity>(new UpdateEntity(2, 2, "Bob"), new List<FieldDescriptor> { new FieldDescriptor(typeof(UpdateEntity).GetProperty("Description")!) })
            };
        }

        private IEnumerable<RowUpdate<UpdateCompositeKeyEntity>> GetValidCompositeKeyEntities()
        {
            return new List<RowUpdate<UpdateCompositeKeyEntity>>
            {
                new RowUpdate<UpdateCompositeKeyEntity>(new UpdateCompositeKeyEntity(1, 2, 1, "Alice"), new List<FieldDescriptor> { new FieldDescriptor(typeof(UpdateEntity).GetProperty("Name")!) }),
                new RowUpdate<UpdateCompositeKeyEntity>(new UpdateCompositeKeyEntity(3, 4, 3, "Bob"), new List < FieldDescriptor > { new FieldDescriptor(typeof(UpdateEntity).GetProperty("Description")!) })
            };
        }
