namespace Rowbot.Pipelines.Runner.DependencyResolution;

internal sealed class EntityDependencyResolver
{
    public List<List<Pipeline>> Resolve(List<Pipeline> pipelineGroups)
    {
        var graphNodes = pipelineGroups.Select(x => new EntityGraphNode(x)).ToList();

        var connectorGroups = ResolveDependencies(graphNodes);

        return connectorGroups.Select(x => x.Select(c => c.Pipeline).ToList()).ToList();
    }
